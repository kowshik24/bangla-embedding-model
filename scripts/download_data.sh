...
dataset = load_dataset('csebuetnlp/paraphrase_identification_bn', trust_remote_code=True)
dataset.save_to_disk('data/raw/paraphrase_bn')

# Download Bangla NLI data (if available)
print('Downloading Bangla NLI dataset...')
try:
    nli_dataset = load_dataset('csebuetnlp/xnli_bn', trust_remote_code=True)
...
sts_dataset = load_dataset('mteb/sts12-sts', trust_remote_code=True)
